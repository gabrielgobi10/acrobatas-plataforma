...
function ResumoCard({
  ...
  return (
    ...
    >
      ...
      <p className="text-base sm:text-xl font-semibold text-zinc-900 dark:text-zinc-100">{valor}</p>
    </motion.div>
  );
}
...
export default function DocumentosProfissionaisEmpresa() {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      ...
      try {
        ...
        // 4) documentos (PAINEL EMPRESA) -> empresa_docs_prof_v
        ...
        (docsDb || []).forEach((d: any) => {
          ...
          // remover contactos de emergência
          ...
          // regras fixas para docs Acrobatas
          if (
          ...
          mappedDocs.push({
            id: d.doc_id, // aqui vem da view empresa_docs_prof_v
        ...
        const profList = Array.from(profMap.values()).sort((a, b) => a.nome.localeCompare(b.nome, "pt"));
  ...
  const categorias = useMemo(
    () =>
      Array.from(new Set(docsDoProf.map((d) => d.categoria).filter(Boolean) as string[])).sort((a, b) =>
        a.localeCompare(b),
      ),
    [docsDoProf],
  );
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-semibold text-[11px] line-clamp-1 text-sm">{p.nome}</span>
                      <span
          ...
          {!profSelecionado ? (
            ...
            <>
              <div className="mb-4">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-base sm:text-lg font-semibold">Documentos de {profSelecionado.nome}</h2>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-6">
                <ResumoCard titulo="Válidos" valor={resumoSelecionado.validos} cor="text-green-500" icone={<CheckCircle2 size={18} />} />
                <ResumoCard titulo="Pendentes" valor={resumoSelecionado.pendentes} cor="text-yellow-500" icone={<AlertTriangle size={18} />} />
                <ResumoCard titulo="Vencidos" valor={resumoSelecionado.vencidos} cor="text-red-500" icone={<XCircle size={18} />} />
              </div>

              <div className="mb-8">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-zinc-600 dark:text-zinc-400">Completude do perfil de documentos</span>
                ...
                <div className="h-2.5 rounded-full bg-zinc-200 dark:bg-zinc-800 overflow-hidden">
                  <div className="h-full bg-blue-600 rounded-full transition-all" style={{ width: `${resumoSelecionado.completion}%` }} />
                </div>
              </div>
                        ...
                        >
                          <div className="flex justify-between items-start gap-3 mb-1">
                            <div className="flex-1">
                              <p className="font-medium text-zinc-800 dark:text-zinc-200 text-sm">{doc.nome}</p>
                              <div className="flex items-center gap-2 mt-[2px] flex-wrap">
                                <span className="text-[11px] text-zinc-500 dark:text-zinc-400">{doc.categoria || "—"}</span>
                          ...
                          <div className="flex gap-3 mt-3">
                            <button
                              ...
                              className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-md text-xs transition ${
                                doc.url ? "bg-blue-600/90 text-white hover:bg-blue-700" : "bg-zinc-400 text-white opacity-70"
                              }`}
                            >
                              <Eye size={14} /> Ver
                            </button>
