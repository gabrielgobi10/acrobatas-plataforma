...
====================== */
type Props = {
  /** Quando estiver no Admin (ou quiser forçar), passe o ownerId do profissional alvo */
  ownerId?: string | null;
  perfil?: {
    id?: string | null;
    nome?: string | null;
...
export default function PortfolioTab({ perfil, ownerId: ownerIdProp }: Props) {
  // Dono do portfólio (pode vir via prop, via perfil ou cair no user logado)
  const [ownerId, setOwnerId] = useState<string | null>(
    ownerIdProp ?? (perfil?.id ? String(perfil.id) : null)
  );

  useEffect(() => {
    (async () => {
      // Se veio pela prop ou pelo perfil, prioriza isso
      if (ownerIdProp ?? perfil?.id) {
        setOwnerId(ownerIdProp ?? String(perfil?.id));
        return;
      }
      // Senão, usa o user atual (painel do próprio)
      const { data, error } = await supabase.auth.getUser();
      if (!error && data?.user?.id) setOwnerId(data.user.id);
    })();
  }, [ownerIdProp, perfil?.id]);
  ...
  const [newFolderVisibility, setNewFolderVisibility] =
    useState<"publico" | "privado">("publico");
  ...
  useEffect(() => {
    async function load() {
      ...
      if (error) {
        console.error("Erro ao carregar pastas de portfólio:", error);
        setErrorMsg("Não foi possível carregar as pastas.");
        setFolders([]);
      } else {
  ...
  useEffect(() => {
    async function loadMedia(folder: Folder) {
      ...
      const { data, error } = await supabase
        ...
        .select("id, tipo, url, thumb_url, titulo, descricao, created_at, path")
        .eq("folder_id", folder.id)
        .order("created_at", { ascending: false });

      if (error) {
        console.error("Erro ao carregar arquivos de portfólio:", error);
  ...
  const filteredMedia = useMemo(() => {
    ...
    if (term) {
      items = items.filter((m) =>
        [m.titulo ?? "", m.descricao ?? ""].join(" ").toLowerCase().includes(term)
      );
    }
    // capa primeiro
  ...
  async function handleConfirmCreateFolder() {
    ...
    if (!uid) {
      ...
      if (error) {
        ...
        setErrorMsg("Não foi possível identificar o utilizador. Inicie sessão novamente.");
        return;
      }
    ...
    setFolders((prev) => [data as unknown as Folder, ...prev]);
    setSavingFolder(false);
  ...
  async function toggleVisibility(folder: Folder) {
    ...
    try {
      ...
      if (selectedFolder?.id === folder.id)
        setSelectedFolder({ ...folder, visibilidade: next });

      const { error } = await supabase
        .from("portfolio_pastas")
        .update({ visibilidade: next })
        .eq("id", folder.id);
  ...
  async function deleteFolderConfirmed() {
    ...
    try {
      ...
      if (paths.length) {
        ...
        if (rmErr) console.warn("Falha ao remover do storage (ignorada):", rmErr.message);
      }
      ...
      const { error: delFolderErr } = await supabase
        .from("portfolio_pastas")
        .delete()
        .eq("id", folder.id);
  ...
  async function uploadFiles(files: FileList, targetFolder: Folder) {
    ...
    try {
      ...
      const uploads = Array.from(files).map(async (file) => {
        ...
        const path = `${owner}/${targetFolder.id}/${Date.now()}_${Math.random()
          .toString(36)
          .slice(2)}.${ext}`;
        ...
        const tipo: "image" | "video" = file.type.startsWith("video")
          ? "video"
          : "image";
      ...
      setErrorMsg("Falha ao enviar um ou mais arquivos.");
    } finally {
      setUploading(false);
      setUploadPct(0);
  ...
  async function handleDeleteMedia(media: Media) {
    ...
    try {
      ...
      // se era capa, limpar ou mover para próxima foto
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
        <div className="flex flex-1 items-center gap-2">
          ...
          >
            ...
            <span className="text-xs opacity-70 hidden sm:block">
              {sort.by === "nome" ? "Nome" : "Data"}
            </span>
          </button>

          <button
            onClick={() =>
              setSort((s) => ({ ...s, by: s.by === "nome" ? "created_at" : "nome" }))
            }
            className="px-3 py-2 rounded-xl bg-black/5 ring-1 ring-black/10 hover:bg-black/10
        ...
        <div className="flex items-center justify-between md:justify-end gap-2">
          ...
          <button
            onClick={() =>
              selectedFolder ? openFilePicker(selectedFolder) : setShowCreateModal(true)
            }
            disabled={uploading}
      ...
      {errorMsg && (
        <p className="text-xs text-red-600 bg-red-500/10 rounded-lg px-3 py-2 dark:text-red-400">
          {errorMsg}
        </p>
      )}
          ...
          {items.map((f) => (
            ...
            >
              ...
              <div className="col-span-3 flex items-center justify-end gap-2">
                <button className="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/10" title="Upload rápido" onClick={() => onQuickUpload(f)}>
                  <Upload className="w-4 h-4" />
                </button>
                <button className="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/10" title="Renomear" onClick={() => onRename(f)}>
                  <Pencil className="w-4 h-4" />
                </button>
                <button className="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/10" title="Alternar visibilidade" onClick={() => onToggleVisibility(f)}>
      ...
      {items.map((f) => (
        <motion.div
          ...
          className="group relative rounded-xl overflow-hidden
                     ...
                     hover:shadow-md
                     dark:ring-white/10 dark:bg-white/[0.03]"
          onDragOver={(e) => {
            e.preventDefault();
            e.dataTransfer.dropEffect = "copy";
          }}
        ...
        >
          <div className="relative">
            <button
              onClick={() => onOpen(f)}
              className="w-full h-40 block bg-black/5 dark:bg-white/5"
              title="Abrir pasta"
            >
              {f.capa_url ? (
                <img src={f.capa_url} className="w-full h-full object-cover" />
            ...
            <button
              ...
              className="absolute bottom-2 left-2 px-2 py-1 rounded-md
                         bg-white/85 text-slate-900 backdrop-blur text-xs ring-1 ring-black/10 hover:bg-white
          ...
          <div className="p-3 space-y-2">
            ...
            {f.descricao && (
              <div className="text-xs opacity-70 line-clamp-2">{f.descricao}</div>
            )}
...
function MediaGrid({
  ...
  if (view === "list") {
    return (
      <>
            ...
            {items.map((m, idx) => (
              <div key={m.id} className="grid grid-cols-12 items-center px-3 py-3 hover:bg-black/[0.04] dark:hover:bg-white/[0.06]">
                <button onClick={() => setViewer({ index: idx })} className="col-span-6 flex items-center gap-3 text-left">
                  <div className={`w-14 h-10 rounded-lg overflow-hidden flex items-center justify-center bg-black/5 dark:bg-white/5 ${m.url === coverUrl ? "ring-2 ring-primary" : "ring-1 ring-black/10 dark:ring-white/10"}`}>
                    {m.tipo === "image" ? (
        ...
        <AnimatePresence>
          {viewer && (
            <Lightbox items={items} startIndex={viewer.index} onClose={() => setViewer(null)} />
          )}
        </AnimatePresence>
      </>
    );
  ...
  // GRID (badges não bloqueiam clique -> pointer-events-none)
  return (
    <>
      <div
          ...
          return (
            <motion.div
              ...
              className={[
                ...
                isCover
                  ? "ring-2 ring-primary shadow-[0_0_0_3px_rgba(59,130,246,0.25)]"
                  : "ring-1 ring-black/10 dark:ring-white/10",
              ].join(" ")}
            >
              {/* clique abre lightbox */}
              <button
                onClick={() => setViewer({ index: idx })}
                className="block w-full h-44 bg-black/5 dark:bg-white/5"
                title="Abrir"
              >
                {m.tipo === "image" ? (
                  <img src={m.url} className="w-full h-full object-cover" />
              ...
              {/* Badges (não interceptam o clique) */}
              ...
              {/* Ribbon diagonal e gradiente – decoração, não clicável */}
              {isCover && (
                <>
                  <div className="pointer-events-none absolute -left-10 -top-3 rotate-[-16deg] z-10">
                    <div className="px-5 py-1 text-[11px] font-semibold bg-primary text-primary-foreground shadow-lg">
                      CAPA
                    </div>
                  </div>
  ...
  return (
    <div
      className="w-full rounded-2xl border-2 border-dashed
                 border-black/15 bg-black/[0.02] py-14 flex flex-col items-center justify-center text-center
    ...
    >
      ...
      <p className="opacity-70 text-sm max-w-md mt-1">
        Você também pode selecionar do dispositivo.
      </p>

      <button
        onClick={onPickFiles}
        disabled={uploading}
        className="mt-4 px-4 py-2 rounded-xl bg-primary text-primary-foreground disabled:opacity-60"
      >
  ...
  return (
    <ModalShell onClose={onClose}>
      ...
      <input
        ...
        className="w-full rounded-xl px-3 py-2 text-sm outline-none
                   bg-white ring-1 ring-black/10 focus:ring-2 focus:ring-primary/40
                   dark:bg-white/[0.06] dark:text-white dark:ring-white/15"
      />
...
function EmptyState({
  ...
  return (
    <div className="w-full rounded-2xl border border-dashed
                    border-black/15 bg-white
                    py-14 flex flex-col items-center justify-center text-center
                    dark:border-white/15 dark:bg-white/[0.02]">
