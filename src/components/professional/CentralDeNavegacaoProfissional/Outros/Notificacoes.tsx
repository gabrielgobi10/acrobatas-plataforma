// src/components/professional/CentralDeNavegacaoProfissional/Outros/Notificacoes.tsx
"use client";
...
export default function Notificacoes({ onVoltar }: Props) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) {
        console.error("Erro ao buscar usuario para notificacoes:", error);
        return;
      }

      if (ok && data) setProfile({ id: data.id });
    })();
    return () => {
      ok = false;
    };
  ...
  const fetchNotis = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, tipo, titulo, conteudo, icone, url_destino, lida, criado_em, empresa_id, usuario_id"
      )
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("notis_profissional_page")
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        (payload: any) => {
          if (payload.eventType === "INSERT")
            setNotis((prev) => [payload.new as Noti, ...prev]);
          else if (payload.eventType === "UPDATE")
            setNotis((prev) =>
              prev.map((n) =>
                n.id === payload.new.id ? (payload.new as Noti) : n
              )
            );
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="leading-tight min-w-0">
                ...
                <div className="text-[11px] opacity-70">
                  ...
                  {unreadCount === 1 ? "" : "s"} • {notis.length} no total
                </div>
              </div>
            </div>
